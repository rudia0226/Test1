...
FROM(  
    ...
    FROM (
            ...
            FROM SALE_TBL    
            WHERE   AREA_CD = NVL(:ARG_AREA, AREA_CD)   AND  ROWNUM <= NVL(:ARG_NUM, 1000000)
            GROUP BY AREA_CD
                    ,REGION_AREA
                    ,PROD_ID                
         )TA, CD_TBL TB
...
WHERE GR = DECODE(:ARG_GR, 'D', '00', 'S', '01', 'T', '11') 
ORDER BY  
         DECODE(AREA_CD, 10, :ARG_SEOUL, 20, :ARG_KYUNGKI, 30, :ARG_INCHEON, 40, :ARG_PYUNGTEK, 50, :ARG_ULSAL, 60 , :ARG_DAEJOEN, 70, :ARG_BUSAN, 80, :ARG_GWANJU)  -- || AREA_CD
         , AREA_CD          
