...
IS 
   ...
   V_BACK_COLOR    VARCHAR2(50);
   
BEGIN  

SELECT  MAX(DECODE(CD_ID,'TOP_COLOR',CMT)) TOP_COLOR
           ,MAX(DECODE(CD_ID,'LAST_COLOR',CMT)) LAST_COLOR
           ,MAX(DECODE(CD_ID,'BACK_COLOR',CMT)) V_BACK_COLOR
           INTO V_TOP_COLOR,V_LAST_COLOR, V_BACK_COLOR
FROM CD_TBL
WHERE CD_ID IN ('TOP_COLOR','LAST_COLOR','BACK_COLOR') ;
  ...
  CASE WHEN INSTR(V_COL,'¡ã') > 0 THEN 
       ...
       WHEN INSTR(V_COL,'*') = 0 THEN 
             V_RET_STR := q'[<td style="background-color: ]'||V_BACK_COLOR||q'[;">]'||V_COL||'</td>';
        ELSE
