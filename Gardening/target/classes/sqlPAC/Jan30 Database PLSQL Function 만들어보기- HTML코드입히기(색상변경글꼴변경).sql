...
CREATE OR REPLACE FUNCTION F_DIA_HTML (V_COL IN VARCHAR)
RETURN VARCHAR2
IS
	V_STR VARCHAR2(1000);
	V_TOP_COLOR VARCHAR2(50);
...
BEGIN 
SELECT MAX(DECODE(CD_ID, 'V_TOP_COLOR', CMT)) TOP_COLOR 
			,  MAX(DECODE(CD_ID, 'V_LAST_COLOR', CMT)) LAST_COLOR 
			,  MAX(DECODE(CD_ID, 'V_BG_COLOR1', CMT)) BG_COLOR1
			,  MAX(DECODE(CD_ID, 'V_BG_COLOR2', CMT)) BG_COLOR2 
FROM F_HTML
WHERE CD_ID IN ('V_TOP_COLOR','V_LAST_COLOR','V_BG_COLOR1','V_BG_COLOR2')
