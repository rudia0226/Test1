...
-- 56page : 지역별 제품 판매 순위에 기호 넣고 row별 tot값으로 각 컬럼 퍼센트 매기기

CREATE VIEW V_DIA_HW01 AS 
SELECT * 
FROM DUAL 
/*   */
...
-- 71page : user의 생성일 소멸일이 같은 경우 중복으로 처리 후 두개의 행을 하나로 합침. (반복시 여러개 합치기)

CREATE VIEW V_DIA_HW02 AS 
SELECT * 
FROM DUAL 
/*   */
...
(
		SELECT 	NO  ,  NM
						,  ST_DT
						,  ET_DT
						, LAG(ET_DT) OVER(PARTITION BY NO ORDER BY  ST_DT ) LAG_ET 
						, DECODE( SIGN(ST_DT - NVL((LAG(ET_DT) OVER(PARTITION BY NO ORDER BY  ST_DT)),0) ), 1 , 1,  0 ) SEQ  
			FROM ORG_TBL
			ORDER BY NO, ST_DT   
		)  
	)
